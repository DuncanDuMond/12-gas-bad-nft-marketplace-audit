...
ghost mathint listingUpdatesCount {
    ...
    // init_state refers to the initial state of the ghost (initial state will be 0)
...
rule calling_any_function_should_result_in_each_contract_having_the_same_state(method f, method f2, address listingAddr, uint256 tokenId, address seller){
    // 1. Going to call the same function on both contracts
    // 2. Compare the getter functions of each contract to ensure they are in the same state
    env e;
    calldataarg args;
